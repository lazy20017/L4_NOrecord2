...
extern float ADC1_Filer_value_buf[ADC1_COLM];
...
//发送12个周期的有效值,共36个数据
extern float WAVE_half_ave_Current2[12][3]; //利用三列存，平均，有效，最大，12个周期的每个周期的值
extern float WAVE_all_ave_Current2[12][3]; //利用三列存，平均，有效，最大
extern float WAVE_all_ave_E_Field2[12][3]; //利用三列存，平均，有效，最大

extern float WAVE_half_ave_Current3[12][3]; //利用三列存，平均，有效，最大，12个周期的每个周期的值
extern float WAVE_all_ave_Current3[12][3]; //利用三列存，平均，有效，最大
extern float WAVE_all_ave_E_Field3[12][3]; //利用三列存，平均，有效，最大
...
void my_fun_101send_Alarm_status_data(UART_HandleTypeDef* USARTx, uint8_t my_status, uint8_t my_contorl_byte)
{
    ...
    my_data[3] = (my_tim6_count>>8);  //计时高字节
		...
		if(my_cyc_alarm_status==1 && my_contorl_byte==0x02)
		{
			if(my_test_value==0)
			{
			my_data[0]=0XF0;  //周期发送录波数据数据,周期报警数据，调试使用的
			my_test_value=0XF0;	
