...
uint8_t  my_sys_time_sample=17;  //录波数据进行计算有效值的时间间隔，默认是17秒
...
void HAL_TIM_PeriodElapsedCallback2(TIM_HandleTypeDef *htim)
{
    ...
    else if(htim == &htim6) //TIM6为1s一次，为了响应中断
    {
          ...
          printf("==Current_exit_status=%d,e_filed_exit_status=%d,my_sys_start_status=%d  time6=%d,reset_count=%d\n"
            ...
            my_Time_Cyc_exit_add = my_wave_write_add;
						...
						case 0x01:
								...
								break;
						case 0x02:
								my_cyc_delay=my_tim6_count+8;
								break;
						case 0x03:
								my_cyc_delay=my_tim6_count+13;
								break;
						default:
								my_cyc_delay=0;
						}
        ...
        if(my_Time_Cyc_exit_Status==0 && my_tim6_count%my_sys_time_sample==0 && my_tim6_count!=0 && my_tim6_count%my_cyc_time_count!=0 && my_CC1101_all_step==0) //
        {
...
//				my_fun_give_Queue(&myQueue01Handle, my_step); //发送报警
				...
				//=======电池直流电源的控制====CC1101，如果电压过低停止CC1101============
					if(ADC1_Filer_value_buf[6] <(float)3.6)  //停止CC1101
					{
						CC1101_PWR_OFF;//CC1101断电
						
					}
					else if (ADC1_Filer_value_buf[6] <(float)3.8) //开启CC1101
					{
