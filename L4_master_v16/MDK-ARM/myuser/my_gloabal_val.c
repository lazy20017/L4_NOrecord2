...
float ADC2_Filer_value_buf_2[ADC2_COLM][3]= {0};  //AD转换后的物理值，12个周期的平均值，有效值，最大值，第1行为全波电流，第2行为电场，第3行为半波电流
float ADC2_Filer_value_buf_3[ADC2_COLM][3]= {0};  //三级缓冲，故障瞬间的换成后的数据


uint16_t ADC1_GetValue[ADC1_ROW][ADC1_COLM]= {0}; //用于保存采集的值,M个通道,N次
uint16_t ADC1_GetValue_Aver[ADC1_COLM]= {0}; //用于保存平均值
float ADC1_Filer_value_buf[ADC1_COLM]= {0};  //用来存储ADC1采样到的的DC直流数据，共7个通道，真实值，转换后的
...
uint16_t my_cyc_time_count=123;//313;  //周期发送数据时间，秒为单位
