...
extern float my_i_ratio_value;
extern float my_E_ratio_value;
extern float MY_VDD;
extern float my_all_a_adjust; //实验数据，301/387获得的，在300A电流时刻，实际值，与ADC测量值的比较值
extern float my_adjust_300_a; //y=x*a+b,最小二乘法，的系数a，b，x为ADC测量值经过校正后的值，利用二乘法进行二次校正
extern float my_adjust_300_b;

extern float my_adjust_50_a;
extern float my_adjust_50_b;

extern float my_adjust_5_a;
extern float my_adjust_5_b;

extern float my_I_100A_Radio;

extern float my_i_5a_radio;//0.857517365; 
extern float my_i_50a_radio;//0.770811922;
extern float my_i_300a_radio;//0.770811922;


//extern int16_t my_dianliu_exit_add;
extern uint16_t my_wave_write_add;
//extern uint16_t my_Time_Cyc_exit_add;
extern float ADC2_Filer_value_buf_2[][3];
...
*/
float my_DAC_Line_I=0; //在DAC设置期间获得的电流值
float my_DAC_Line_Efild=0; //同上，电场值
float my_adjust_value_V=0.00;
float my_counst_value=0.04;// 电场参考电压的恒定偏差，0.004对应1A电流，小于0.09V就频繁进中断，默认设置0.09
void my_fun_DAC_evref_auto_ajust(void)
{
		float temp_i = 0, temp_e = 0;
    uint16_t my_temp_16=0;
    uint16_t my_12v_int=0;
