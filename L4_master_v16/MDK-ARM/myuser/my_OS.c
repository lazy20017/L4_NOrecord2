...
extern uint8_t my_use_alarm_rec_data_status ; //是否进行周期录波数据发送，1为发送，0为不发送
extern uint8_t my_use_alarm_rec_data_status_Efild ;
...
uint8_t my_fun_RX_CC1101_text0_RX_OK(void)
{
		...
		else if(my_CC1101_COM_Fram_buf[1] == 0x20 && my_CC1101_all_step == 0x0054)
    {
		...
		if(my_use_alarm_rec_data_status==0 && my_use_alarm_rec_data_status_Efild==0 && my_CC1101_COM_Fram_buf[1] == 0x20 && my_CC1101_all_step == 0x0052)
		{
			my_zsq_ALarm_send_status=0;
				if(my_CC1101_Sleep_status==1)
					CC1101SetSleep();
        printf("====@@@@ CC1101 ALarm TIME FINISH!!==yaoxin=\n\n");
