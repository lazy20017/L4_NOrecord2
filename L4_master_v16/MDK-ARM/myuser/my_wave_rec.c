...
//========
float my_all_a_adjust = 0.76560196; //半波1.42655781;//全波0.798980075;    //1.42655781; // 0.798980075=301/368          301/387=0.7838541670; //实验数据，301/387获得的，在300A电流时刻，实际值，与ADC测量值的比较值
//295、395.7815=0.745360761
float my_adjust_300_a = 1.054171799;//1.009700101;//1.007328; //y=x*a+b,最小二乘法，的系数a，b，x为ADC测量值经过校正后的值，利用二乘法进行二次校正
float my_adjust_300_b = -12.56341994;//-1.587214894;//-2.42403;

float my_adjust_50_a = 0.986329196; //0.994672291; //1.00022;
float my_adjust_50_b = 0.686363636; //-0.144475163;// -2.19103;

float my_adjust_5_a = 1.059577357; //0.994672291; //1.00022;
float my_adjust_5_b = -0.272350023; //-0.144475163;// -2.19103;

float my_I_100A_Radio = 1.0; //标准线上电流为100A时的校正系数，算法，默认值为1，100A/测得数据的值
float my_i_ratio_value = 1000; //1/2.0*1500; //电流变比系数//采样电阻为2欧姆，变比为1500:1,最终用的1000
float my_value_daya = 2.0; //加法校正系数，默认2.0

//+++++++++++++++++++++++
//float my_i_5a_radio = 5.02 / 7.61; //5.08/6.26;//0.662251656;//0.857517365;   //5.08--6.26  303---393
//float my_i_50a_radio = 50.9 / 71.06;
//float my_i_300a_radio = 306.1 / 412.6; //293.4/377.354;//0.748091603;//0.770811922;

//float my_10A_gatedata = 15.14;
//float my_100A_gatedata = 139.92;

extern float my_i_5a_radio ; //5.08/6.26;//0.662251656;//0.857517365;   //5.08--6.26  303---393
extern float my_i_50a_radio ;
extern float my_i_300a_radio; //293.4/377.354;//0.748091603;//0.770811922;

extern float my_10A_gatedata ;
extern float my_100A_gatedata ;

//+++++++++++++++++++++++++=
//在乘上一个100A时得到的校正系数。

float my_E_ratio_value = 1000; //电场变比系数
...
float WAVE_half_ave_Current2[12][3] = {0}; //利用三列存，平均，有效，最大，12个周期的每个周期的值
float WAVE_all_ave_Current2[12][3] = {0}; //利用三列存，平均，有效，最大
float WAVE_all_ave_E_Field2[12][3] = {0}; //利用三列存，平均，有效，最大

//3级缓冲，线上真实电流值
float WAVE_half_ave_Current3[12][3] = {0}; //利用三列存，平均，有效，最大，12个周期的每个周期的值
float WAVE_all_ave_Current3[12][3] = {0}; //利用三列存，平均，有效，最大
float WAVE_all_ave_E_Field3[12][3] = {0}; //利用三列存，平均，有效，最大

//================
extern float MY_VDD;
extern uint16_t ADC2_GetValue[ADC2_ROW][ADC2_COLM];//用于保存采集的值,M个通道,N次,录波
extern float ADC2_Filer_value_buf_2[ADC2_COLM][3];
extern float ADC2_Filer_value_buf_3[ADC2_COLM][3];
...
extern float my_DAC_Line_I; //在DAC设置期间获得的电流值
extern float my_DAC_Line_Efild; //同上，电场值
...
void my_adc2_convert2(uint8_t my_status)
{
    ...
    float temp1 = 0, temp2 = 0, temp3 = 0,max3=0;
...
//////////////============================
float my_all_current_aver0 = 0;
uint8_t fun_Judege_It_cache3(void)
{
    //double my_all_current_aver0 = 0;

    float my_all_current_aver1[4] = {0};
    float my_all_current_aver2[8] = {0};

    float my_half_current_aver1[4] = {0}; //故障前4个半波
    float my_half_current_aver2[8] = {0}; //半波ADC采样后转换后得到的电流值
    float my_E_fild_aver0 = 0;
    float my_E_fild_aver1[4] = {0};
    float my_E_fild_aver2[8] = {0};
    float my_cmpar_value[3][12] = {0}; //0行全波电流，1行电场，2行半波电流
    ...
    for(ii = 0; ii < 8; ii++)
    {
        ...
        my_E_fild_aver2[ii] =( WAVE_all_ave_E_Field3[4 + ii][1] / (float)E_cell); //中断后8个周期的每个周期的电场值
    }
...
uint8_t fun_Judege_It_cache2(void)
{
    ...
    float	my_all_current_aver2[12] = {0};
    float  my_half_current_aver2[12] = {0}; //半波ADC采样后转换后得到的电流值
    float  my_E_fild_aver0 = 0;
    float  my_E_fild_aver2[12] = {0};
    float  my_cmpar_value[3][12] = {0}; //0行全波电流，1行电场，2行半波电流
    ...
    for(ii = 0; ii < 12; ii++)
    {
        ...
        my_E_fild_aver2[ii] = WAVE_all_ave_E_Field2[ii][1] /(float) E_cell;
    }

    //电场校正成相对量
    my_E_fild_aver0 = WAVE_all_ave_E_Field3[0][1] / (float)E_cell;
